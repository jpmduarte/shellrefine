...
# Retest of both phases with a lower lr and a much longer cosine schedule
# (250 epochs). Phase 1 was still improving steadily at epoch 50 (no
# plateau, train/val loss close together) so this mainly gives it more room
# to converge. Phase 2's val_patch_dice plateaued by epoch 7 and oscillated
# 0.910-0.917 without a clear trend through epoch 50 -- the classic symptom
# of lr too high for the task's fine detail, hence the bigger cut and the
# much longer schedule there.
python pipeline.py \
