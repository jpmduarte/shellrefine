...
def get_splits(val_fraction: float = 0.2, seed: int = 42):
    ...
    paths = sorted(glob.glob(os.path.join(SHELLS, "*.npz")))
    ...
    print(f"Saved splits to {SPLITS_JSON}  (train={len(train_paths)}, val={len(val_paths)})")
